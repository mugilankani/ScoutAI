...
export const findCandidatesAndFetchProfiles = async (req, res) => {
  ...
  if (!SERPAPI_KEY || !GOOGLE_AI_KEY || !APIFY_TOKEN) {
    console.error('Missing API keys. Ensure SERPAPI_API_KEY, GOOGLE_API_KEY, and APIFY_API_TOKEN are set.');
    return res.status(500).json({ error: 'API keys not configured. Please check server environment variables.' });
  }
  ...
  try {
    ...
    console.log(`Screened ${filteredCandidates.length} candidates based on the recruiter query.`);
    ...
    if (linkedInUrlsToScrape.length === 0) {
      console.warn('No valid LinkedIn profile URLs found to scrape after initial screening.');
      // If no profiles to scrape, return the already screened candidates.
      return res.status(200).json({
        message: 'No LinkedIn profiles found for detailed scraping. Returning initially screened candidates.',
        summary: summary,
    ...
    console.log(`Successfully fetched ${fullLinkedInProfiles.length} full LinkedIn profiles from Apify.`);
    ...
    res.status(200).json({
      ...
      fullProfilesData: fullLinkedInProfiles, // The detailed LinkedIn profiles from Apify
    });
