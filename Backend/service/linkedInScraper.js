 ...
 */
export async function fetchLinkedInProfiles(profileUrls, apifyToken) {
  ...
  console.log(
    `[LinkedInScraperClient] Initiating Apify scraper for ${profileUrls.length} profiles.`
  );
  ...
  try {
    ...
    if (response.data && Array.isArray(response.data)) {
      console.log(
        `[LinkedInScraperClient] Successfully fetched ${response.data.length} profiles from Apify.`
      );
      return response.data; // The data directly maps to the desired profile structure
    } else {
      console.warn(
        "[LinkedInScraperClient] Apify response did not contain an array of profile data or was empty:",
        response.data
      );
    ...
    if (axios.isAxiosError(error)) {
      console.error(
        `[LinkedInScraperClient] Error fetching LinkedIn profiles from Apify: ${error.message}`
      );
      if (error.response) {
        // Log detailed response data from Apify if available
        console.error(
          `[LinkedInScraperClient] Apify Response Data: ${JSON.stringify(
            error.response.data
          )}`
        );
        console.error(
          `[LinkedInScraperClient] Apify Status: ${error.response.status}`
        );
      }
    } else {
      // Log any other unexpected errors
      console.error(
        "[LinkedInScraperClient] An unexpected error occurred while scraping LinkedIn profiles:",
        error
      );
    }
    ...
    throw new Error(
      `Failed to scrape LinkedIn profiles: ${error.message || "Unknown error"}`
    );
  }
}
