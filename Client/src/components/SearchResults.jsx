...
const SearchResults = () => {
  ...
  const [loadingProgress, setLoadingProgress] = useState(passedInitialData?.progress || 0);
  const [loadingMessage, setLoadingMessage] = useState(passedInitialData?.statusMessage || "Loading search status...");
  ...
  const pollForResults = (sId) => {
    ...
    const poll = async () => {
      if (!isLoading && pollingTimerRef.current) { // Check isLoading state
        console.log("SEARCH_RESULTS: isLoading is false, stopping poll.");
        clearInterval(pollingTimerRef.current);
        return;
      }
      if (totalPollingTimeRef.current > MAX_POLL_DURATION) {
        console.error("SEARCH_RESULTS: Max polling duration reached for search:", sId);
        setSearchError("Search took too long and timed out. Please try again or check back later.");
      ...
      try {
        const response = await axiosInstance.get(`/candidates/search/status/${sId}`);
        ...
        if (data.status === "pending" || data.status === "processing") {
          ...
          const finalCandidates = data.results?.finalResult || data.results?.candidates || [];
          setCandidates(finalCandidates);
          setSearchSummary(data.results?.summary || null);
          setTimeout(() => setIsLoading(false), 500);
        } else if (data.status === "failed") {
          console.error("SEARCH_RESULTS: Search failed as per status API:", data.error);
          setSearchError(data.error || data.statusMessage || "An unknown error occurred during the search.");
          setLoadingMessage(`Error: ${data.statusMessage || "Search failed."}`);
          setIsLoading(false);
          clearInterval(pollingTimerRef.current);
        } else {
          console.warn("SEARCH_RESULTS: Unknown search status received:", data.status, "Continuing poll.");
          pollingTimerRef.current = setTimeout(poll, POLLING_INTERVAL);
        }
      } catch (err) {
        console.error("SEARCH_RESULTS: Polling error:", err);
        if (err.response?.status === 404) {
            setSearchError("Search ID not found. It might be invalid or expired.");
            setLoadingMessage("Error: Search not found.");
            setIsLoading(false);
            clearInterval(pollingTimerRef.current);
        } else {
            setLoadingMessage("Connection issue while checking status, retrying...");
  ...
  useEffect(() => {
    console.log("SEARCH_RESULTS: useEffect triggered. paramSearchId:", paramSearchId);
    clearInterval(pollingTimerRef.current);
    setSearchError(null);
    ...
    if (paramSearchId) {
      ...
      if (passedInitialData && (passedInitialData.status === 'pending' || passedInitialData.status === 'processing')) {
        console.log("SEARCH_RESULTS: Has paramSearchId and initialData, starting poll.");
        setIsLoading(true); // Ensure loading is true if we start polling
        pollForResults(paramSearchId);
      } else if (passedInitialData && passedInitialData.status === 'completed') {
        console.log("SEARCH_RESULTS: Has paramSearchId and initialData is 'completed'.");
        setLoadingMessage("Search complete!");
        setLoadingProgress(100);
        const finalCandidates = passedInitialData.results?.finalResult || passedInitialData.results?.candidates || [];
        setCandidates(finalCandidates);
        setSearchSummary(passedInitialData.results?.summary || null);
        setIsLoading(false);
      } else if (passedInitialData && passedInitialData.status === 'failed') {
        console.log("SEARCH_RESULTS: Has paramSearchId and initialData is 'failed'.");
        setSearchError(passedInitialData.error || passedInitialData.statusMessage || "Search failed.");
        setLoadingMessage(`Error: ${passedInitialData.statusMessage || "Search failed."}`);
        setIsLoading(false);
      } else {
        // No initial data from state, or it's an old link. Fetch status immediately.
        console.log("SEARCH_RESULTS: Has paramSearchId but no valid initialData, fetching initial status.");
        setIsLoading(true);
        axiosInstance.get(`/candidates/search/status/${paramSearchId}`)
          .then(response => {
            const data = response.data;
             if (queryDisplay.current === "your query" && data.query) {
            ...
            if (data.status === "completed") {
              ...
              // ... (handle failed)
              setSearchError(data.error || data.statusMessage || "Search failed.");
              setLoadingMessage(`Error: ${data.statusMessage || "Search failed."}`);
              setIsLoading(false);
          ...
          .catch(err => {
            console.error("SEARCH_RESULTS: Error fetching initial status for existing search:", err);
            if (err.response?.status === 404) {
                setSearchError("Search ID not found. It might be an invalid or old link.");
                setLoadingMessage("Error: Search not found.");
            } else {
                setSearchError("Could not load search status. Please check your connection.");
        ...
        console.warn("SEARCH_RESULTS: Navigated with query in state but no paramSearchId. This flow should be initiated from HomePage.");
        setSearchError("Search not properly initiated. Please start a new search.");
        setIsLoading(false);
    }
    else {
    ...
    return () => {
      console.log("SEARCH_RESULTS: Cleanup function called, clearing polling timer.");
      clearInterval(pollingTimerRef.current);
    };
          ...
          <div className="text-center max-w-xl w-full px-4">
            <div className="flex justify-center mb-6">
              <svg // Your spinner SVG
                xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-blue-600 animate-spin">
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-4xl flex flex-col mx-auto pt-10 p-6">
        <div className="mb-6">
          <div className="flex gap-4 items-center justify-between mb-4"> {/* Changed items-between */}
            <div>
              ...
              {searchSummary && (
                 <p className="text-gray-500 text-xs mt-1 italic">{searchSummary}</p>
              )}
            </div>
            <button
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          {candidates.length > 0 ? (
            candidates.map((candidate, index) => (
              <TalentRow key={candidate.public_identifier || candidate.id || candidate.url || `candidate-${index}`} talent={candidate} />
            ))
          ) : (
            <div className="p-8 text-center text-gray-500">
              <p>{searchError ? "Could not display results due to an error." : "No candidates found for this search."}</p>
              <button
                onClick={handleNewSearch}
        ...
        {candidates.length > 0 && (
          <div className="flex mt-6 ml-auto gap-4 text-sm text-gray-600">
            <span>Found {candidates.length} candidate{candidates.length === 1 ? "" : "s"}</span>
          </div>
        )}
      </div>
    </div>
